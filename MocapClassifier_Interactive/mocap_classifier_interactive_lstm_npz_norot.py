...
# Pre-computed feature size (22 joints * 9 features per joint [pos(3)+vel_pos(3)+acc_pos(3)])
num_features = len(mocap_joint_indices) * 9
...
save_stats_path = "../MocapClassifier/results_Yurika_Mediapipe_norot_30_v2/stats"
model_weights_file = "../MocapClassifier/results_Yurika_Mediapipe_norot_30_v2/weights/classifier_weights_epoch_200.pth"
...
class OSCWorker(QtCore.QThread):
    ...
    def set_running(self, state):
        ...
        if not state:
            # Clear buffer on stop so old data doesn't corrupt new sessions
    ...
    def osc_pos_handler(self, address, *args):
        ...
        # Append directly to buffer instead of using an intermediary variable
        self.buffer_pos.append(pos_data)

        # Trigger inference once the position buffer is full
        if len(self.buffer_pos) == self.buffer_pos.maxlen:
            pos = np.array(self.buffer_pos)
            
            # Compute position derivatives over the window
            ...
            with torch.no_grad():
                ...
                print("batch_x_norm s ", batch_x_norm.shape)
